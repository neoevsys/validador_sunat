...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/*">
        ...
        <xsl:call-template name="isTrueExpresion">
            ...
            <xsl:with-param name="node" select="cac:DespatchSupplierParty/cbc:CustomerAssignedAccountID" />
            <xsl:with-param name="expresion" select="$numeroRuc != cac:DespatchSupplierParty/cbc:CustomerAssignedAccountID" />
        </xsl:call-template>
		...
		<xsl:if test="$cacAdditionalDocumentReference">
			<xsl:for-each select="$cacAdditionalDocumentReference">
		       ...
		       <!-- PAS20211U210700059 - Excel v7 - Corrección del ERR-2756 para que el conteo de la cantidad de caracteres no se altere por acentos o caracteres especiales-->
					<!--xsl:call-template name="regexpValidateElementIfExist">
           ...
           <xsl:choose>        	
              <xsl:when test="string-length(./cbc:ID) &gt; 20 or string-length(./cbc:ID) &lt; 1 " >
	       	       <xsl:call-template name="regexpValidateElementIfExist">
			 		          <xsl:with-param name="errorCodeValidate" select="'2756'"/>
					          <xsl:with-param name="node" select="./cbc:ID" />
		...
		<!-- PAS20221U210700001 Se cambia la validación 2760 para que verifique el catálogo 006 -->			
		<!--xsl:call-template name="existAndRegexpValidateElement">
		...
		<xsl:call-template name="findElementInCatalog">
			<xsl:with-param name="errorCodeValidate" select="'2760'"/>
			<xsl:with-param name="idCatalogo" select="$cbcDeliveryCustomerAccountID/@schemeID"/>
    ...
    <!-- PAS20211U210700059 - Excel v7 - Corrección del ERR-2762 para que el conteo de la cantidad de caracteres no incluya acentos -->
		<!--xsl:call-template name="existAndRegexpValidateElement">
    ...
    <xsl:call-template name="existElement">
		...
		<xsl:choose>        	
       <xsl:when test="string-length($cbcDeliveryCustomerName) &gt; 100 or string-length($cbcDeliveryCustomerName) &lt; 1 " >
	       	<xsl:call-template name="regexpValidateElementIfExist">
			 		   <xsl:with-param name="errorCodeValidate" select="'2762'"/>
					   <xsl:with-param name="node" select="$cbcDeliveryCustomerName" />
					   <xsl:with-param name="regexp" select="true()" />
       ...
       <xsl:otherwise>					
		...
		<xsl:if test="$cbcShipmentHandlingCode[text() = '09' or text() = '9']">
		...
		<xsl:if test="$cbcShipmentHandlingCode[text() = '08' or text() = '8']">
			<!-- Se corrige validacion 2768 - PAS20201U210400041 -->
      <xsl:if test="count($cacAdditionalDocumentReference/cbc:DocumentTypeCode[text()='01' or text()='04']) = 0">
				<xsl:call-template name="rejectCall">
		...
		<xsl:call-template name="existElement">
			...
			<xsl:with-param name="node" select="cac:Shipment/cac:Delivery/cac:DeliveryAddress/cbc:StreetName"/>
		</xsl:call-template>
		
        <xsl:choose>        	
       		<xsl:when test="string-length(cac:Shipment/cac:Delivery/cac:DeliveryAddress/cbc:StreetName) &gt; 100 or string-length(cac:Shipment/cac:Delivery/cac:DeliveryAddress/cbc:StreetName) &lt; 1 " >
	        	<xsl:call-template name="regexpValidateElementIfExist">
		...
		<xsl:call-template name="existElement">
			<xsl:with-param name="errorCodeNotExist" select="'2775'"/>
			<xsl:with-param name="node" select="cac:Shipment/cac:OriginAddress/cbc:ID"/>
		</xsl:call-template>
		...
		<xsl:for-each select="$cacShipmentOriginAddress">
	        ...
	        <xsl:choose>        	
	       		<xsl:when test="string-length(./cbc:StreetName) &gt; 100 or string-length(./cbc:StreetName) &lt; 1 " >
		        	<xsl:call-template name="regexpValidateElementIfExist">
						<xsl:with-param name="errorCodeValidate" select="'2778'"/>
						<xsl:with-param name="node" select="./cbc:StreetName" />
						<xsl:with-param name="regexp" select="true()" />
      ...
      <!--xsl:call-template name="regexpValidateElementIfExist">
      		...
      		<xsl:choose>
			   <xsl:when test="string-length(cac:Item/cbc:Name) &gt; 250">
		        <xsl:call-template name="isTrueExpresion">
		            <xsl:with-param name="errorCodeValidate" select="'2782'" />
		            <xsl:with-param name="node" select="cac:Item/cbc:Name" />
      ...
      <!-- PAS20211U210700059 - Excel v7 - Corrección de OBS-2783 para que no acepte espacios, tab, etc-->
      <xsl:choose>
			   <xsl:when test="string-length(cac:Item/cac:SellersItemIdentification/cbc:ID) &gt; 16">
		        <xsl:call-template name="isTrueExpresion">
    ...
    <xsl:template match="cbc:Note">
		...
		<xsl:choose>        	
       		<xsl:when test="string-length(text()) &gt; 250 or string-length(text()) &lt; 1 " >
	        	<xsl:call-template name="regexpValidateElementIfExist">
					<xsl:with-param name="errorCodeValidate" select="'4186'"/>
