...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template match="/*">
	  ...
	  <xsl:call-template name="regexpValidateElementIfExist">
        ...
        <!-- Fin PAS20211U210700059 - Excel v7 - Se agrega ERR-3308 y ERR-3309-->
    ...
    <!--<xsl:choose>
      ...
      <xsl:otherwise>
		...
		<!--PAS20221U210600205-->	
		<xsl:if test="$tipoOperacion ='0113' and cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']] and cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]/cbc:TaxAmount &gt; 0">
			<xsl:call-template name="isTrueExpresion">
				<xsl:with-param name="errorCodeValidate" select="'4290'" />
				<xsl:with-param name="node" select="cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]/cbc:TaxAmount" />
				<xsl:with-param name="expresion" select="($SumatoriaIGV + 1 ) &lt; $SumatoriaIGVCalculado or ($SumatoriaIGV - 1) &gt; $SumatoriaIGVCalculado" />
        ...
        <xsl:if test="cac:Party/cac:PartyIdentification/cbc:ID != '-'">
	        <xsl:choose>
				...
				<xsl:when test="(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID ='4') or (cac:Party/cac:PartyIdentification/cbc:ID/@schemeID ='7') or
								...
								(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID ='D') or (cac:Party/cac:PartyIdentification/cbc:ID/@schemeID ='E') or
								(cac:Party/cac:PartyIdentification/cbc:ID/@schemeID ='F') or (cac:Party/cac:PartyIdentification/cbc:ID/@schemeID ='G')">
        ...
        <xsl:if test= "cbc:DocumentTypeCode != '02' and cbc:DocumentTypeCode != '03'">
	        ...
	        <xsl:call-template name="existAndRegexpValidateElement">
              ...
              <!--xsl:with-param name="regexp" select="'^(0[145]|99)$'"/-->
	            <xsl:with-param name="regexp" select="'^(0[234589]|99)$'"/>
              <xsl:with-param name="isError" select ="false()"/>
	            <xsl:with-param name="descripcion" select="concat('Documento Relacionado : ', cbc:DocumentTypeCode,'-',cbc:ID)"/>
	        </xsl:call-template>
	    </xsl:if>
...
<!--         </xsl:if> -->
        ...
        <xsl:if test="$tipoOperacion = '2105'">     
			  ...
			  <xsl:if test="cac:Item/cac:AdditionalItemProperty">
				...
				<xsl:call-template name="isTrueExpresion">
				  <xsl:with-param name="errorCodeValidate" select="'2595'" />
					<xsl:with-param name="node" select="cac:Item/cac:AdditionalItemProperty/cbc:NameCode" />
				  <xsl:with-param name="expresion" select="count(cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '7017']) &lt; 1" />
				</xsl:call-template>
				
				<xsl:call-template name="isTrueExpresion">
				  <xsl:with-param name="errorCodeValidate" select="'2596'" />
					<xsl:with-param name="node" select="cac:Item/cac:AdditionalItemProperty/cbc:NameCode" />
				  <xsl:with-param name="expresion" select="count(cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '7018']) &lt; 1" />
				</xsl:call-template>
				
				<xsl:call-template name="isTrueExpresion">
				  <xsl:with-param name="errorCodeValidate" select="'2597'" />
					<xsl:with-param name="node" select="cac:Item/cac:AdditionalItemProperty/cbc:NameCode" />
				  <xsl:with-param name="expresion" select="count(cac:Item/cac:AdditionalItemProperty/cbc:NameCode[text() = '7019']) &lt; 1" />
				</xsl:call-template>				
...
<!-- 	    PAS20191U210000012 -->		
        ...
        <xsl:if test="$codigoTributo = '1000' or $codigoTributo = '1016'">
	    	<xsl:call-template name="isTrueExpresion">
	            <xsl:with-param name="errorCodeValidate" select="'3111'" />
	            <xsl:with-param name="node" select="cbc:TaxAmount" />
	            <!-- <xsl:with-param name="expresion" select="cbc:TaxableAmount &gt; 0 and $tipoOperacion != '0113' and cbc:TaxAmount = 0" /> PAS20191U210100194-->
	            <xsl:with-param name="expresion" select="cbc:TaxableAmount &gt; 0.06 and $tipoOperacion != '0113' and cbc:TaxAmount = 0" /> <!--  PAS20191U210100194-->
	        	<xsl:with-param name="descripcion" select="concat('Error en la linea: ', $nroLinea, ' Tributo: ', $codigoTributo)"/>
	    	</xsl:call-template>
	    </xsl:if>
		...
		<xsl:if test="$codigoTributo != '7152'">
		    <!-- PAS20191U210100194 INICIO JOH-->
		    <xsl:if test="$codigoTributo != '2000' and $codigoTributo != '9999' and $codigoTributo != '' and cbc:TaxableAmount &gt; 0">
...
<!--           PAS20191U210000012 add 7152       -->
    ...
    <xsl:template match="cac:AllowanceCharge" mode="linea">
        ...
        <xsl:variable name="MontoCalculado" select="round(cbc:BaseAmount * cbc:MultiplierFactorNumeric * 100) div 100"/>
...
<!--             	<xsl:call-template name="regexpValidateElementIfExist">
    ...
    <xsl:template match="cac:TaxTotal" mode="cabecera">
        ...
        <!-- PAS20211U210700059 - Excel v7 - Se redondea la variable totalBaseIGVCalculado a dos decimales -->
        ...
        <!-- PAS20211U210700059 - Excel v7 - Se usa la variable nueva $totalISCxLinear-->
        <xsl:call-template name="isTrueExpresion">
    ...
    <xsl:template match="cac:AllowanceCharge" mode="cabecera">
        ...
        <!-- <xsl:when test="$codigoCargoDescuento = '00' or $codigoCargoDescuento = '01' or $codigoCargoDescuento = '02' or $codigoCargoDescuento = '03'"> -->
		<!-- PAS20211U210700059 - Excel v7 - Se agrega el código "20"-->
        <xsl:if test="$codigoCargoDescuento = '02' or $codigoCargoDescuento = '03' or $codigoCargoDescuento = '04' or $codigoCargoDescuento = '05' or $codigoCargoDescuento = '06' or $codigoCargoDescuento = '20'"> 
			<xsl:call-template name="isTrueExpresion">
				<xsl:with-param name="errorCodeValidate" select="'3114'" />
				<xsl:with-param name="node" select="cbc:ChargeIndicator" />
				<xsl:with-param name="expresion" select="cbc:ChargeIndicator/text() = 'true'" />
